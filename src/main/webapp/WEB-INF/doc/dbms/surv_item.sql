...
COMMENT ON COLUMN surv_choice.surv_ans_1 is '답변';
...
commit;
2) 목록
SELECT SURV_ITEM_NO, SURV_NO, mem_no, SURV_item_SEQNO, surv_item_ans, surv_item_cnt, surv_item_passwd
FROM SURV_ITEM
ORDER BY SURV_item_NO DESC;

SELECT SURV_ITEM_NO, SURV_NO, SURV_item_SEQNO, surv_item_ans, surv_item_cnt, surv_item_passwd
FROM SURV_ITEM
WHERE surv_no = 1
ORDER BY SURV_item_NO DESC;
...
update surv_item
set surv_item_cnt = surv_item_cnt +1
where surv_item_no=2 AND surv_no=1;



SELECT s.surv_no as s_surv_no, s.surv_title as s_surv_title,
           i.surv_no as surv_no, i.SURV_item_SEQNO as surv_item_seqno, i.surv_item_ans as surv_item_ans,
           i.surv_item_cnt as surv_item_cnt, i.surv_item_passwd as surv_item_passwd
FROM surv s, surv_item i
WHERE s.surv_no = i.surv_no AND i.surv_no=1;



3) 조회
SELECT SURV_ITEM_NO, SURV_NO, SURV_item_SEQNO, surv_item_ans_1, surv_item_ans_2, surv_item_ans_3, surv_item_ans_4, surv_item_cnt, surv_item_passwd
FROM SURV_ITEM
where surv_no=1;

3-1) join list
SELECT s.surv_no as s_surv_no, s.surv_title as s_surv_title,
           i.surv_item_no as surv_item_no, i.surv_no as surv_no, i.surv_item_ans_1 as surv_item_ans_1, i.surv_item_ans_2 as surv_item_ans_2,
           i.surv_item_ans_3 as surv_item_ans_3, i.surv_item_ans_4 as surv_item_ans_4
    from surv s, surv_item i
    where i.surv_no = s.surv_no and s.surv_no=2
    order by i.surv_no asc, s.surv_no asc;
    
    
3-2) surv+surv_item list (by surv_no)
select s.surv_no as s_surv_no, s.surv_title as s_surv_title, 
...
4) 수정
update surv_item
set surv_item_seqno=12, surv_item_ans_1='답변수정', surv_item_ans_2='답변수정', surv_item_ans_3='답변수정', surv_item_ans_4='답변수정'
where surv_no=2;
