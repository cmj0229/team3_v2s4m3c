...
<body>
  ...
  <DIV class='title_line'>자주 묻는 질문</DIV>
  ...
  <form name='frm' id='frm' method='get' action='./list.do'>
    ...
    <ASIDE style='float: right;'>
      ...
      <c:choose>
        <c:when test="${param.word != '' }">
          ...
          <input type='text' name='word' id='word'
            value='${param.word }' style='width: 35%;'>
        </c:when>
        <c:otherwise>
          <%-- 검색하지 않는 경우 --%>
      ...
      <c:if test="${param.word.length() > 0 }">
        <button type='button' onclick="location.href='./list.do?word='">검색
          취소</button>
      </c:if>
        ...
        <c:when test="${sessionScope.id_admin != null}">
          ...
          <tbody>
            <c:forEach var="faqVO" items="${list }">
              ...
              <tr>
                <td class="td_bs">${faqVO.faq_seqno}
                  [${faqVO.faqno}]</td>
